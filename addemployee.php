...
if ($id != 0) {
    $sql = "SELECT id, name, address, email, dob, contact, gender FROM employee WHERE id='$id'";
    $results = $conn->query($sql);

    if ($results && $results->num_rows > 0) {
        $data = mysqli_fetch_assoc($results);

        // Assign values with fallback defaults
        $name = $data['name'] ?? '';
    ...
    <div class="container">
        ...
        <main>
            ...
            <form action="employee_controller.php" method="post" onsubmit="return validate()">
                ...
                <input type="submit" value="Submit" style="background-color: green; font-size: 20px; padding: 10px 20px; border-radius: 5px; color: white; border: none;">
            </form>
        </main>
    </div>
</body>
